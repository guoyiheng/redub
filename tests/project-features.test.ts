...
describe('项目置顶与属性功能', () => {
  ...
  it('语音指令、引用上文和引用下文使用文档标签，只有合成文本会被朗读', async () => {
    ...
    expect(prompt).toMatch(/^\*指令：\* \[#.*用颤抖沙哑、带着崩溃与绝望的哭腔说/)
    expect(prompt).toContain('*引用上文：* [#你还好吗？]')
    expect(prompt).toContain('*引用下文：* [#别再说了。]')
    expect(prompt).toContain('*合成文本：* 我逆转时空九十九次救你。')
