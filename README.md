...
3. 在台词工作区核对原文。手动点击“翻译台词”后只执行翻译，完成后先核对译文，再自行发起配音。翻译会调用已配置的翻译接口并消耗额度。
4. 每句台词都可以单独打开生成设置并生成配音，也可以点击“批量配音”统一生成。配音结束后停下来逐句试听，不会自动合成或导出。AI 配音默认调用火山 Audio `seed-audio-1.0`；微软 TTS 选项无需 API Key，但需要网络连接。音视频项目使用原声参考前必须完成人声分离。
5. 预览页随时可播放已完成的片段，混合背景音、已有配音与未配音的原声。合成、导出分别手动发起；导出可选择仅音轨或视频成片，视频沿用原格式、保留原音轨并追加配音轨，视频不重新编码。

手动提交的处理任务先写入本地 SQLite 并获得独立任务 ID，再由后台队列执行；预览音轨独立读取已完成内容并更新缓存，不排入处理队列。刷新页面不会取消手动任务；右下角任务管理器可查看详情与全部历史任务，详情链接刷新后仍可恢复。旧版本遗留的跨步骤自动任务会标记为“已取消”，保留处理记录；重试、跳过或继续项目都不会自动启动下一步。
