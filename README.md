...
在“设置 → 任务与识别”中分别配置翻译和配音并发数，默认翻译 10、配音 5，两类额度互不占用。批量翻译和配音按句并行处理；本机预处理与合成独立调度。调整额度仅影响后续启动的任务，不中断正在执行的任务。右下角任务面板分为“进行中”和“已完成”两个标签，任务失败、跳过和取消的记录归入“已完成”并显示实际状态，支持一键重试失败项。
