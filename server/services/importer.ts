...
const inputSchema = z.object({ name: z.string().trim().min(1).max(120), text: z.string().max(200000).optional(), targetLanguage: z.string().trim().min(1).max(40).default('中文'), sourceLanguage: z.enum(['auto', 'zh', 'en', 'ja', 'ko', 'es', 'fr', 'de', 'ru']).default('auto') })
const extensions = ['.mp4', '.mov', '.mkv', '.webm', '.avi', '.mp3', '.wav', '.m4a', '.flac', '.ogg', '.aac', '.txt', '.srt', '.vtt']
export async function importProject(event: H3Event) {
  const id = randomUUID(), dir = await projectDir(id)
  let temp: string | undefined
  try {
    let data: z.infer<typeof inputSchema>, kind: MediaKind = 'text', sourcePath: string | null = null, duration = 0
    if (getRequestHeader(event, 'content-type')?.startsWith('multipart/form-data')) {
      const staging = join(dataDir, 'uploads')
      await mkdir(staging, { recursive: true })
      const form = formidable({ uploadDir: staging, maxFileSize: 10 * 1024 ** 3, maxFiles: 1, maxFieldsSize: 1024 * 1024 })
      form.on('fileBegin', (_name, file) => { temp = file.filepath })
      ...
      data = inputSchema.parse(Object.fromEntries(Object.entries(fields).map(([key, values]) => [key, values?.[0]])))
      if (['.txt', '.srt', '.vtt'].includes(extension)) {
        ...
        duration = info.duration; kind = info.video ? 'video' : 'audio'
      }
    ...
    await db.transaction(async tx => {
      await tx.insert(projects).values({ id, name: data.name, kind, sourcePath, duration, sourceLanguage: data.sourceLanguage, targetLanguage: data.targetLanguage, createdAt: Date.now(), updatedAt: Date.now() })
