...
function binary(name: 'ffmpeg' | 'ffprobe') {
  ...
  } catch { /* Fall back to the system installation in development. */ }
  return name
}
...
export function runProcess(command: string, args: string[], timeoutMs = 60 * 60 * 1000) {
  return new Promise<string>((resolvePromise, reject) => {
    ...
    child.on('error', e => { clearTimeout(timer); reject(new Error(`无法启动 ${command.split(/[\\/]/).pop()}：${e.message}`)) })
    child.on('close', code => {
      clearTimeout(timer)
      if (code === 0) resolvePromise(stdout)
      else reject(new Error(timedOut ? '处理超时，可调整素材长度后重试' : `处理失败：${stderr.slice(-2000) || `退出码 ${code}`}`))
    })
  })
}
export const ffmpeg = (args: string[]) => runProcess(binary('ffmpeg'), ['-hide_banner', '-loglevel', 'error', '-y', ...args])
export async function probe(path: string) {
  const result = JSON.parse(await runProcess(binary('ffprobe'), ['-v', 'error', '-show_format', '-show_streams', '-of', 'json', path], 30000))
  const duration = Number(result.format?.duration)
  if (!Number.isFinite(duration) || duration <= 0) throw new Error('无法读取素材时长，请检查文件是否损坏')
  if (!result.streams.some((s: { codec_type: string }) => s.codec_type === 'audio')) throw new Error('素材中没有可用音轨')
...
export async function cutAudio(source: string, target: string, start: number, duration: number) {
  await ffmpeg(['-ss', String(start), '-i', source, '-t', String(duration), '-ar', '24000', '-ac', '1', '-c:a', 'pcm_s16le', target])
}
export function tempoFilters(ratio: number) {
  const values: number[] = []
  while (ratio > 2) { values.push(2); ratio /= 2 }
  while (ratio < .5) { values.push(.5); ratio /= .5 }
  values.push(ratio)
...
export async function alignAudio(source: string, target: string, duration: number) {
  ...
  await ffmpeg(['-i', source, '-af', `${tempoFilters(info.duration / duration)},apad,atrim=0:${duration},afade=t=in:d=0.008,afade=t=out:st=${Math.max(0, duration - .008)}:d=0.008`, '-ar', '48000', '-ac', '2', target])
}
...
export async function mediaHealth() {
  const check = async (command: string, args: string[]) => {
    try { await runProcess(command, args, 15000); return true } catch { return false }
  }
  const [ffmpegReady, ffprobeReady, modelsReady] = await Promise.all([
    check(binary('ffmpeg'), ['-version']), check(binary('ffprobe'), ['-version']),
