...
export function parseScript(input: string): TextSpan[] {
  const text = input.replace(/^\uFEFF/, '').replace(/\r/g, '').trim()
  ...
  const timed = blocks.flatMap(block => {
    ...
    if (n < 0) return []
    const match = lines[n]!.match(/(\d{1,2}:\d{2}(?::\d{2})?[.,]\d{3})\s*-->\s*(\d{1,2}:\d{2}(?::\d{2})?[.,]\d{3})/)
    if (!match) throw new Error('字幕时间格式无效')
    return [{ start: seconds(match[1]!), end: seconds(match[2]!), text: lines.slice(n + 1).join('\n').replace(/<[^>]*>/g, '') }]
  })
  if (text.includes('-->') && !timed.length) throw new Error('未识别到有效字幕')
  let cursor = 0
  const spans = timed.length ? timed : text.split('\n').filter(Boolean).map(line => {
    const start = cursor
  ...
  for (let i = 0; i < spans.length; i++) {
    const s = spans[i]!
    if (s.end <= s.start || s.end - s.start > 120 || s.text.length > 2800 || !s.text.trim()) throw new Error('每条台词需包含文字，时长为 0–120 秒且不超过 2800 字')
