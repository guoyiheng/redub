...
import { db } from '../db'
import { projects, segments, channels } from '../db/schema'
import { getProject, getSegments, getChannel, getSettings, invalidateOutput } from './store'
import { assetPath, projectDir, extractAudio, cutAudio, ffmpeg, alignAudio, python, runProcess, localModel } from './media'
...
export async function executeJob(job: Job, progress: (value: number, message: string) => Promise<void>) {
  ...
  const update = async (value: Partial<typeof p>) => { await db.update(projects).set({ ...value, updatedAt: Date.now() }).where(eq(projects.id, p.id)) }
  ...
  if (job.stage === 'separate') {
    ...
    await runProcess(python(), ['-m', 'demucs.separate', '--two-stems', 'vocals', '-n', 'htdemucs', '-d', 'cpu', '-o', join(dir, 'stems'), join(dir, 'original.wav')])
  ...
  if (job.stage === 'segment') {
    ...
    if (existing.length) throw new Error('项目已有片段；为保留编辑内容，不自动覆盖。可手动调整片段或重新导入项目')
    await progress(10, '检测人声起止时间')
    const spans: { start: number; end: number }[] = await localModel('segment', { audio: assetPath(p.vocalsPath) }, join(dir, 'vad.json'))
    if (!spans.length) throw new Error('未检测到人声，请检查素材或手动添加片段')
    const rows: (typeof segments.$inferInsert)[] = []
    for (let i = 0; i < spans.length; i++) {
      const span = spans[i]!, id = randomUUID()
    ...
    await db.transaction(async tx => { for (const row of rows) await tx.insert(segments).values(row) })
  }
  if (job.stage === 'transcribe') {
    ...
    const result: { id: string; text: string }[] = await localModel('transcribe', { segments: inputs, model: settings.whisperModel, language: p.sourceLanguage === 'auto' ? null : p.sourceLanguage }, join(dir, 'transcript.json'))
    for (const row of result) {
      if (!lines.some(s => s.id === row.id)) throw new Error('识别返回未知片段')
      await db.update(segments).set({ text: row.text, translation: '', generatedPath: null, generatedHash: null }).where(eq(segments.id, row.id))
    }
    await invalidateOutput(p.id)
  }
  if (job.stage === 'translate') {
    const lines = (await getSegments(p.id)).filter(s => s.enabled)
    if (!lines.length || lines.some(s => !s.text.trim())) throw new Error('请先识别或填写所有启用片段的原文')
    const [selected] = await db.select().from(channels).where(eq(channels.type, 'openai')).orderBy(channels.id)
    const channel = await getChannel(selected?.id || 'translation-default')
    for (let i = 0; i < lines.length; i += 20) {
      const batch = lines.slice(i, i + 20)
      const result = await translateLines(batch, p.targetLanguage, channel)
      await db.transaction(async tx => {
        for (const s of batch) await tx.update(segments).set({ translation: result.get(s.id)!, generatedPath: null, generatedHash: null }).where(eq(segments.id, s.id))
      })
      await progress(Math.round(100 * Math.min(i + 20, lines.length) / lines.length), `已翻译 ${Math.min(i + 20, lines.length)} / ${lines.length}`)
    }
    await invalidateOutput(p.id)
  }
  if (job.stage === 'synthesize') {
    const channel = await getChannel(p.channelId)
    const lines = (await getSegments(p.id)).filter(s => s.enabled && (!job.segmentId || s.id === job.segmentId))
    if (!lines.length) throw new Error('没有启用的配音片段')
    await invalidateOutput(p.id)
    for (let i = 0; i < lines.length; i++) {
      const s = lines[i]!, hash = synthesisHash(s, channel)
      if (s.generatedPath && s.generatedHash === hash && existsSync(assetPath(s.generatedPath))) continue
      await progress(Math.round(i / lines.length * 100), `正在配音 ${i + 1} / ${lines.length}，保留已成功片段`)
      const output = rel(`voice-${s.id}-${randomUUID()}.mp3`)
      const result = await synthesizeSpeech(s, channel, assetPath(output))
      await db.update(segments).set({ generatedPath: output, generatedHash: hash, generatedDuration: result.duration, subtitle: result.subtitle }).where(eq(segments.id, s.id))
    }
  }
  if (job.stage === 'mix') {
    await invalidateOutput(p.id)
    ...
    if (enabled.some(s => !s.generatedPath)) throw new Error('部分启用片段尚未生成配音，请完成配音或关闭这些片段的替换开关')
    if (p.kind !== 'text' && (!p.audioPath || (enabled.length && !p.backgroundPath))) throw new Error('缺少原始或背景音轨，请完成人声与背景分离')
    const chunks: string[] = []
    let cursor = 0
    const chunk = async (start: number, end: number, segment?: Segment) => {
      ...
      if (segment) {
        ...
        else await ffmpeg(['-ss', String(start), '-t', String(length), '-i', assetPath(p.backgroundPath!), '-i', aligned,
          '-filter_complex', '[0:a][1:a]amix=inputs=2:duration=longest:normalize=0,alimiter=limit=0.98:latency=1', '-t', String(length), '-ar', '48000', '-ac', '2', '-c:a', 'pcm_s16le', path])
      } else if (p.audioPath) {
        await ffmpeg(['-ss', String(start), '-i', assetPath(p.audioPath), '-t', String(length), '-ar', '48000', '-ac', '2', '-c:a', 'pcm_s16le', path])
      } else {
        await ffmpeg(['-f', 'lavfi', '-i', 'anullsrc=r=48000:cl=stereo', '-t', String(length), '-c:a', 'pcm_s16le', path])
      }
      chunks.push(path)
    }
    for (let i = 0; i < enabled.length; i++) {
      const line = enabled[i]!
      if (line.start < cursor || line.end > duration + .01) throw new Error('片段时间重叠或超出素材长度，请调整后重试')
      await chunk(cursor, line.start)
      await chunk(line.start, line.end, line)
      cursor = line.end
      await progress(Math.round((i + 1) / enabled.length * 90), `对齐并合并片段 ${i + 1} / ${enabled.length}`)
    }
    ...
    await writeFile(join(dir, 'subtitles.srt'), subtitleText(lines.map(s => ({ ...s, text: s.enabled ? s.translation || s.text : s.text }))))
  ...
  if (job.stage === 'preview') {
    ...
    if (p.kind === 'video') {
      ...
      await ffmpeg(['-i', assetPath(p.sourcePath!), '-i', assetPath(p.mixedPath), '-map', '0:v:0', '-map', '1:a:0', '-c:v', 'libx264', '-preset', 'fast', '-crf', '18', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '256k', '-movflags', '+faststart', '-t', String(p.duration), assetPath(output)])
